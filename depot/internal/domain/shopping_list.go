...
func (s ShoppingListStatus) String() string {
	switch s {
	case ShoppingListUnknown, ShoppingListAvailable, ShoppingListAssigned, ShoppingListActive, ShoppingListCompleted,
		ShoppingListCancelled:
		return string(s)
	default:
		return ""
	}
...
func CreateShopping(ID string, orderID string) *ShoppingList {
	return &ShoppingList{
		ID:      ID,
		OrderID: orderID,
